	  ...
	  let newhl = 
		...
		when not(vi.vstorage = Extern) ->
		  ...
		  inss'::inss
              ...
              (* FSL: May need to fix later if we ever use this functionality -- for now, just get past compiler *)
	      | Assert _ | Assume _ | Invariant _ | IAssert _ | Unlock _ | Havoc _ | IAssume _ | Checkpoint _ | 
		    Warn _ | TAssert _ | TAssume _ | TVarDecl _ | FCall _ -> inss) [(List.hd hl)] (List.combine il (List.tl hl))
	  in
